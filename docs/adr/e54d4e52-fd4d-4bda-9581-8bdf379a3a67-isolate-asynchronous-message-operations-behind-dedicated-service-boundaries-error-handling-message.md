...
1. MUST: Error handling for message queue operations must distinguish between synchronous validation failures that block the HTTP response and asynchronous delivery failures that require separate observability mechanisms.

## Policy Block

- MUST Error handling for message queue operations must distinguish between synchronous validation failures that block the HTTP response and asynchronous delivery failures that require separate observability mechanisms.
