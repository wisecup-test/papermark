...
1. MUST: All message queue operations invoked from HTTP route handlers must be encapsulated behind dedicated service boundaries that abstract the messaging implementation from transport-layer concerns.

## Policy Block

- MUST All message queue operations invoked from HTTP route handlers must be encapsulated behind dedicated service boundaries that abstract the messaging implementation from transport-layer concerns.
