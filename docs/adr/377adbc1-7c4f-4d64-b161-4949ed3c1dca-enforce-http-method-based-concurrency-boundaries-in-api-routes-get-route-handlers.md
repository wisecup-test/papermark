...
1. SHOULD: GET route handlers that perform read-only database queries SHOULD use appropriate isolation levels to prevent dirty reads and ensure consistent snapshots.

## Policy Block

- SHOULD GET route handlers that perform read-only database queries SHOULD use appropriate isolation levels to prevent dirty reads and ensure consistent snapshots.
