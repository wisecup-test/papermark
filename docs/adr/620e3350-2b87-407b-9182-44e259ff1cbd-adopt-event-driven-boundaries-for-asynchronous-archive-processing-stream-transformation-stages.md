...
1. MUST: Stream transformation stages that compute derived values MUST use transform streams with chunk-based event handlers to maintain pipeline backpressure

## Policy Block

- MUST Stream transformation stages that compute derived values MUST use transform streams with chunk-based event handlers to maintain pipeline backpressure
