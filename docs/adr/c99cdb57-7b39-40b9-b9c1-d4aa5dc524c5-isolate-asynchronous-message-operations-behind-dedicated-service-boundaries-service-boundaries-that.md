...
1. MUST: Service boundaries that coordinate message queue operations must accept domain-level parameters and must not expose queue-specific configuration, connection details, or protocol semantics to calling code.

## Policy Block

- MUST Service boundaries that coordinate message queue operations must accept domain-level parameters and must not expose queue-specific configuration, connection details, or protocol semantics to calling code.
