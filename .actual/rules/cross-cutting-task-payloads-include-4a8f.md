...
These rules are ALWAYS ACTIVE for all background task definitions that coordinate multi-step workflows, span process or runtime boundaries, or are invoked across service boundaries and runtime environments.

### Rules

- **R-TASK-001** MAY: Task payloads MAY include metadata fields for progress reporting or user-facing status updates.
...
if [ -n "$lint_config" ]; then
  echo "Lint configuration found: $lint_config"
...
- All task payload interfaces are defined in dedicated contract modules separate from implementation
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Type check failures, contract test failures, and missing public API exports are blocking violations. Code review MUST verify all new task definitions export typed payload interfaces and task identifiers as public API.
</enforcement>