...
- **R-TRAVERSE-001** MUST: Queries that traverse relations must specify select clauses for nested entities to prevent transitive over-fetching.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All database queries in public API route handlers must include explicit select clauses for all entities and nested relations before code review approval.
</enforcement>