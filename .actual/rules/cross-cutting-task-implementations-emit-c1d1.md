# Export Typed Task Payloads as Public API Contracts: Task Implementations Emit

These rules are ALWAYS ACTIVE for all background task definitions that coordinate multi-step workflows, task payloads invoked across process or runtime boundaries, asynchronous operations requiring progress tracking or observability correlation, and task contracts consumed by external orchestration systems or SDKs.

### Rules

- **R-TASK-001** SHOULD: Task implementations SHOULD emit structured log events at workflow stage boundaries to enable progress tracking and failure diagnosis.
...
if [ -n "$type_checker_config" ]; then
  ...
  # Execute type checker (tool discovery deferred to project)
fi
...
if [ -n "$test_dir" ]; then
  ...
  # Execute contract tests (tool discovery deferred to project)
fi
...
if [ -n "$lint_config" ]; then
  ...
  # Execute linter (tool discovery deferred to project)
fi
```
...
- All task implementations emit structured log events at workflow stage boundaries with consistent metadata shapes
- Runtime validation is enforced at task entry points with clear error messages for contract violations

<enforcement>
Claude Code MUST NOT skip or defer verification. Type checking in continuous integration pipeline, contract tests validating payload shape conformance, and code review verification of public API exports for new task definitions are mandatory. Type check failures block merge to protected branches. Contract test failures trigger build failure and prevent deployment. Code review process requires explicit approval for task contract changes.
</enforcement>